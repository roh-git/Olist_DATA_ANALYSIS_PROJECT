...
분석에는 2016년 9월 4일부터 2018년 9월 3일까지의 Olist 주문 데이터와 고객·상품·판매자·결제·리뷰·위치 정보를 사용했다. 원천 데이터는 총 9개 테이블이며, 각 테이블은 주문·상품·결제·리뷰처럼 서로 다른 분석 단위를 갖는다.
...
`customers`의 `customer_id`는 주문마다 새로 부여될 수 있으므로 고객 단위 분석에는 `customer_unique_id`를 사용했다. 또한 상품·결제·리뷰 테이블은 주문 하나에 여러 행이 존재할 수 있어, 바로 결합하지 않고 먼저 주문 단위로 집계했다.
...
문제를 정의하고 분석 가설을 수립하기 위해 고객의 반복 구매 구조와 배송 환경을 먼저 확인했다. 시각화는 문제의 규모와 핵심 비교가 필요한 네 항목으로 제한하고, 나머지 결과는 표로 압축했다. 모든 수치는 현재 프로젝트의 주문 상태 필터와 중복 제거 기준으로 다시 계산했다.

#### 1) 반복 구매 고객 비중

![1회 구매 고객과 반복 구매 고객 비중](report_assets/eda_repeat_customer_share.png)

- 분석 고객 94,986명 중 92,099명(96.96%)은 1회만 구매했다.
- 2회 이상 구매한 고객은 2,887명(3.04%)이었다.
- 따라서 문제를 신규 고객의 이탈로 단정하지 않고, **첫 구매 이후 반복 구매로 이어지는 고객이 적다**고 정의했다.
...
- 반복 구매 고객 안에서도 구매 횟수가 빠르게 감소하므로, 전체 평균만으로 고객 행동을 설명하기 어렵다.
...
| 주제              | 결과                                            | 한줄요약(인사이트)                     |
| --------------- | --------------------------------------------- | ------------------------------ |
| 재구매 간격          | 구매 간격 중앙값 69일·30일 이내 반복 구매 696건               | 30일 단기 지표와 60일 이상 보조 지표의 병행 필요 |
...
기초 EDA는 반복 구매가 적다는 핵심 문제와 배송 경험을 살펴봐야 할 이유를 보여주지만 인과관계를 증명하지 않는다. 이 결과를 바탕으로 다음 절에서 문제와 가설을 명확히 정의한다.
...
고객의 3.04%만 두 번 이상 주문했다. 데이터에 가입일이 없으므로 이를 신규 고객의 이탈 문제로 단정하지 않고, **첫 구매 이후 반복 구매로 이어지는 고객이 적다**는 문제로 정의했다.
...
미수령은 전체 저평점 이벤트의 25.2%인 2,324건에서 탐지됐다. 이 중 일부는 실제 분실이나 누락뿐 아니라 배송이 예정일보다 늦어져 리뷰 작성 시점까지 상품을 받지 못한 사례일 수 있다. 따라서 별도로 탐지된 배송 지연 636건도 무시할 수 없는 연관 신호다.
...
하나 이상의 이슈가 탐지된 리뷰는 4,584건으로 49.6%였다. 나머지 4,649건, 즉 50.4%는 현재 키워드 규칙으로 분류되지 않았다. 원문 VOC가 사라진 것은 아니지만, 현재 분류 체계만 사용하면 운영 지표와 개선 과제에 연결되지 못해 사실상 VOC의 절반을 활용하지 못하는 셈이다.
...
우선 KPI는 **VOC 분류 커버리지**, **미분류율**, **신규 이슈 발견 수**, **사람 검수 후 라벨 수정률**로 설정한다. 목표는 모든 리뷰를 억지로 기존 유형에 넣는 것이 아니라, 미분류 리뷰에서 반복되는 새로운 문제를 발견해 VOC 체계를 지속적으로 확장하는 것이다.
